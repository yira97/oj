    ...
    for (let i = 1; i <= maxMove; i++) {
        const positions = ts.reduce((remindPositions, pos) => {
            ...
            [left, right, up, down]
                ...
                .filter(newPos => remindSteps >= leastStep(m, n, newPos.row, newPos.col))
                .filter(newPos => {
                    const found = remindPositions.find(remindPos => remindPos.row === newPos.row && remindPos.col === newPos.col);
                    if (found !== undefined) {
